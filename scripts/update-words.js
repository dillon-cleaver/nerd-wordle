...
async function updateWords() {
  ...
  const buildArgs = shouldValidate ? `--version ${version} --validate` : `--version ${version}`;
  ...
  if (!firestoreSuccess) {
    console.error('❌ Firestore update failed. CDN deployment will be skipped.');
    process.exit(1);
  }
  ...
  if (shouldDeploy) {
    if (environment === 'production') {
      ...
      if (deploySuccess) {
        ...
        console.log(`   CDN URL: https://nerd-word-cfda3.web.app/dict/${version}/words.json`);
        console.log(`   Environment: ${environment}`);
      ...
      console.log('ℹ️  Skipping CDN deployment (not in production environment)');
    }
  } else {
...
if (environment === 'production' && !process.env.CI && !args.includes('--force')) {
